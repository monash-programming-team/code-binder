// DISCLAIMER: DON'T USE THIS UNLESS YOU ABSOLUTELY NEED TO
...
private:
    ...
    void push(int u, int v, int id){
        ct tmp = min(excess[u], edges[id].cap - edges[id].flow);
        excess[u] -= tmp;
        excess[v] += tmp;
        edges[id].flow += tmp;
        edges[id^1].flow -= tmp;
    ...
    void discharge(int u){
        ...
        while (excess[u] > 0){
            for (; current_edge[u] < (int) g[u].size(); current_edge[u]++){
                ...
                if (excess[u] == 0) break; // remain at current_edge
            }
