// Test for vertex biconnectivity -- identifying articulation points
//
// Verdict: AC
//
...
int main() {
  ...
  while (cin >> N, N > 0) {
    ...
    for (int i = 0; i < N; i++) {
      cin >> name;
      ids[name] = (int)names.size();
      names.push_back(name);
    }
    cin >> M;
    for (int i = 0; i < M; i++) {
      cin >> name >> name2;
      b.add_edge(ids[name], ids[name2]);
    }
  
    if (testno > 1) puts("");
    auto res = b.find_components().first;
    
    vector<string> ans;
    for (int i = 0; i < N; i++) {
      if (res[i]) ans.push_back(names[i]);
    }
    sort(ans.begin(), ans.end());
    
    printf("City map #%d: %d camera(s) found\n", testno++,
      (int)count(res.begin(), res.end(), 1));
